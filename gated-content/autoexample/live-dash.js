/* =========================================================================
   live-dash.js — realtime Cell A dashboards (Asset Health + OEE) for the
...
(function(){
  ...
  var MODEL = {
    ...
    robots:[{i:'R1',a:88},{i:'R2',a:83,live:true},{i:'R3',a:71},{i:'R4',a:42,alert:true},{i:'R5',a:80},
            {i:'R6',a:72},{i:'R7',a:62},{i:'R8',a:81},{i:'R9',a:69},{i:'R10',a:84}],
  ...
  function health(){
    ...
    return '<div class="ld"><div class="ld-hd"><h4>Cell A &#183; Base Coat</h4><span class="sub">roll-up '+MODEL.cellAHI+'% &#183; Watch</span></div>'
      ...
      +'<div class="ld-tq"><div class="h">&#9679; R2 live axis torque (Nm) &#183; Ignition &#8594; HighByte &#8594; UNS</div><div class="g">'
      +MODEL.torque.map(function(t,i){return '<div class="ax"><div class="n">A'+(i+1)+'</div><div class="t">'+t.toFixed(1)+'</div></div>';}).join('')
      +'</div></div>'
      +'<div class="ld-flag">&#9650; R4 driving the cell &#8212; 42%, recurring emergency callouts</div></div>';
  }
  function oee(){
  ...
  var BRIDGE = (typeof window!=='undefined' && window.UNS_BRIDGE) || 'https://oem-uns.example';

  var ticks=0;
  var Feed = {
    liveTorque:false,   // true only while fresh r2 messages are arriving
    _lastTorque:0,
    tick:function(){
      ticks++;
      // If the real r2 leg goes quiet (Ignition sim paused / retained value only),
      ...
      // R2 axis torque: driven by the real UNS feed when connected; otherwise
    ...
    // and maps the real r2 axis-torque payload straight into MODEL.torque. The
    ...
    connectUNS:function(){
      ...
      es.addEventListener('uns',function(ev){
        ...
        // R2 base-coat robot: real per-axis max torque, Ignition -> HighByte -> UNS
        if(/\/r2$/i.test(t) && p.torqueMaxA1!=null && !msg.snapshot){
